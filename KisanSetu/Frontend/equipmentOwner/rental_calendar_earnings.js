...
function renderTxnHistory() {
    ...
    list.innerHTML = sorted.map(r => {
        ...
        const asset = fleet.find(a => a.id === r.equipId);
        return `
        <div class="txn-row">
            ...
            <div class="txn-right">
                ...
                ${isConfirmed ? `<button class="txn-dl-btn" onclick="downloadRentalInvoice('${r.id}')" title="Download Invoice">
...
function downloadRentalInvoice(reqId) {
    const req = allRequests.find(r => r.id === reqId);
    if (!req) { alert('Booking not found.'); return; }

    if (!window.KisanInvoice) {
        alert('PDF engine not loaded. Please check your internet and reload.');
        return;
    }

    const asset = fleet.find(a => a.id === req.equipId);

    const btn = document.querySelector(`[onclick="downloadRentalInvoice('${reqId}')"]`);

    KisanInvoice.download('equipment_rental', {
        bookingId:    req.bookingId || req.id,
        equipName:    req.equipName || '—',
        equipType:    asset?.type || req.equipModel || '—',
        ownerName:    document.getElementById('pageTitle')?.closest('.page-header')?.querySelector('.page-sub')?.textContent?.split('·')[0]?.trim() || 'Equipment Owner',
        farmerName:   req.farmerName || '—',
        farmerPhone:  req.farmerPhone || '—',
        farmerVillage: req.farmerVillage || '—',
        startDate:    req.startDate,
        endDate:      req.endDate,
        days:         req.days || 1,
        hoursPerDay:  req.hoursPerDay || 0,
        hourlyRate:   req.hourlyRate || 0,
        totalCost:    req.totalCost || 0,
        purpose:      req.purpose || 'Agricultural Use'
    }, btn);
}
